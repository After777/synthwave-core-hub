...
const handler = async (req: Request): Promise<Response> => {
  ...
  try {
    ...
    const userEmailResponse = await resend.emails.send({
      from: "Rav Labs <onboarding@resend.dev>",
      to: [email],
      ...
      html: `
        ...
        <p>${message}</p>
        <p>Best regards,<br>The Rav Labs Team</p>
      `,
    });

    console.log("Confirmation email sent successfully:", userEmailResponse);

    // Send notification email to Rav Labs
    const notificationEmailResponse = await resend.emails.send({
      from: "Rav Labs Contact Form <onboarding@resend.dev>",
      to: ["contactravlabs@gmail.com"],
